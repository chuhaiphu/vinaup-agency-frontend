...
The `Api`/`Action` segment names the layer; `Public`/`Private` names the auth scope. → [REPOSITORY-PATTERN](pattern/REPOSITORY-PATTERN.md)
...
├── providers/    Client state — (seeded from server component)
...
→ [REPOSITORY-PATTERN](pattern/REPOSITORY-PATTERN.md), [CACHING-PATTERN](pattern/CACHING-PATTERN.md)
...
- Reserved for **per-user, server-seeded client state** (the auth session) — never server entities, and never browser-only shared UI (that is Zustand, §8). Seed from a Server Component; the provider never fetches.
...
- **Reuse cross-app components from `@vinaup/ui`** instead of copy-pasting.
...
| Need                                                | Mechanism                        |
| --------------------------------------------------- | -------------------------------- |
| Pure display / server data, route-scoped            | props, Server Component          |
| Local toggle / transient UI                         | `useState`                       |
| Form + validation                                   | `@mantine/form`                  |
| Browser-only state shared across the tree           | Zustand                          |
| Server data fetched per route                       | Server Component + Server Action |
| Server-seeded client state (auth session)           | client Context provider (seeded) |
