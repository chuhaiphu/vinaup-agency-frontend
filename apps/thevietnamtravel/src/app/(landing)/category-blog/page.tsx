...
async function CategoryBlogPageWrapper({
    searchParamsPromise,
}: {
    ...
    return (
        <Container size="xl" classNames={{ root: classes.pageRoot }} pb="xl" pt={0}>
            <Stack gap="md">
                {/* Banner Section */}
                <Paper radius="md" className={classes.banner} p="20px">
                    <Title order={1} className={classes.bannerTitle} mb="sm">
                        Title Category [h1]
                    </Title>
...
export default function CategoryBlogPage(props: {
    ...
    return (
        ...
        >
            <CategoryBlogPageWrapper searchParamsPromise={props.searchParams} />
        </Suspense>
    );
}
