...
export default async function LandingDiaryDetailPageContent({
  ...
  const additionalImageSlides: SectionCarouselSlide[] = diaryData.additionalImageUrls.map(
    (url) => ({ src: url }),
  );
  ...
  const renderDestinationAndCategory = () => {
    const hasDestinations = diaryData.destinations && diaryData.destinations.length > 0;
