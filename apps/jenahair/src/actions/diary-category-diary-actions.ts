...
function invalidateDiaryCategoryDiaryTags(args?: { diaryId?: string; diaryCategoryId?: string }) {
  updateTag('diaries');
...
export async function createDiaryCategoryDiaryActionPrivate(
  input: CreateDiaryCategoryDiaryRequest,
): Promise<ActionResponse<DiaryCategoryDiaryResponse>> {
  const result = await executeApi(async () => createDiaryCategoryDiaryApiPrivate(input));
  if (result.success) {
...
export async function getDiaryCategoryDiariesByDiaryCategoryIdActionPublic(
  ...
  return executeApi(async () => getDiaryCategoryDiariesByDiaryCategoryIdApiPublic(diaryCategoryId));
}

export async function getAllDiaryCategoryDiariesActionPublic(): Promise<
  ActionResponse<DiaryCategoryDiaryResponse[]>
> {
  'use cache';
  cacheLife('default');
...
export async function updateDiaryCategoryDiaryActionPrivate(
  id: string,
  input: UpdateDiaryCategoryDiaryRequest,
): Promise<ActionResponse<DiaryCategoryDiaryResponse>> {
  const result = await executeApi(async () => updateDiaryCategoryDiaryApiPrivate(id, input));
  if (result.success) {
    invalidateDiaryCategoryDiaryTags();
  }
  return result;
